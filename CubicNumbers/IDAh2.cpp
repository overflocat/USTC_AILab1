...
int main()
{
	...
	int limit = cBegin.cost;
	bool resultFound = false;
	long startTime = clock();

	while ( !resultFound )
	{
		int nextLimit = INT32_MAX;
		set<string> statusList;
		cubQueue.push(cBegin);

		while (!cubQueue.empty())
		{
			tempC1 = cubQueue.top();
			cubQueue.pop();

			if (tempC1.cost > limit)
				nextLimit = nextLimit > tempC1.cost ? tempC1.cost : nextLimit;
			else
			{
				if (tempC1 == cEnd)
				{
